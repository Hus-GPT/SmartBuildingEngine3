...
def _confirm_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton("✅ تأكيد التنفيذ", callback_data="confirm_write"),
...
async def units(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        text = "🏠 لا توجد وحدات مسجلة." if not rows else "🏠 الوحدات:\n" + "\n".join(
            f"• {u.id} — {u.number} — {u.name} ({'شقة' if u.unit_type == 'apartment' else 'محل'})" for u in rows
        )
...
async def add_lease(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    parts = [p.strip() for p in " ".join(context.args).split("|")]
    if len(parts) != 8:
        await update.message.reply_text("الصيغة:\n/addlease unit_id | tenant_id | YYYY-MM-DD | YYYY-MM-DD أو - | rent | deposit | due_day | payment_method")
        return
    try:
        start = date.fromisoformat(parts[2]); end = None if parts[3] == "-" else date.fromisoformat(parts[3])
        rent = Decimal(parts[4]); deposit = Decimal(parts[5]); due = int(parts[6])
        session = _session(context)
        try:
...
async def add_meter(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(parts) != 4 or parts[1].lower() not in {"electricity","water","كهرباء","ماء"}:
        await update.message.reply_text("الصيغة:\n/meter unit_id | electricity/water | YYYY-MM-DD | reading")
        return
    try:
        meter = MeterType.ELECTRICITY if parts[1].lower() in {"electricity","كهرباء"} else MeterType.WATER
        reading_date = date.fromisoformat(parts[2]); value = Decimal(parts[3])
        session = _session(context)
        try:
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "cancel_write":
        ...
        await query.edit_message_text("❌ أُلغيت العملية ولم تُحفظ بيانات.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("↩️ القائمة", callback_data="menu")]])); return
    if data == "confirm_write":
        pending = context.user_data.get("pending_write")
        if not pending:
            await query.edit_message_text("انتهت صلاحية عملية التأكيد. لم يُنفذ شيء."); return
        session = _session(context)
        try:
            ...
            if kind == "unit":
                ...
                obj = service.add_lease(int(pending["unit_id"]), int(pending["tenant_id"]), date.fromisoformat(pending["start"]), None if pending["end"] == "-" else date.fromisoformat(pending["end"]), Decimal(pending["rent"]), Decimal(pending["deposit"]), int(pending["due"]), pending["payment_method"], True)
                message = f"✅ تم إنشاء العقد رقم {obj.id}."
            elif kind == "meter":
                obj = service.add_meter_reading(int(pending["unit_id"]), MeterType(pending["meter_type"]), date.fromisoformat(pending["date"]), Decimal(pending["value"]), True)
                message = f"✅ تم حفظ قراءة العداد رقم {obj.id}."
            else:
                raise BusinessRuleError("Unknown pending operation.")
    ...
    if data == "list_tenants":
        ...
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("↩️ القائمة", callback_data="menu")]])); return
    help_text = {
        "help_add_unit":"/addunit رقم | الاسم | apartment/shop",
        "help_add_tenant":"/addtenant الاسم | الهاتف[, هاتف آخر]",
        "help_add_lease":"/addlease unit_id | tenant_id | YYYY-MM-DD | YYYY-MM-DD أو - | rent | deposit | due_day | payment_method",
        "help_meter":"/meter unit_id | electricity/water | YYYY-MM-DD | reading",
        "invoices_info":"🧾 دورة الفواتير قيد الاستكمال والاختبار؛ لن تُعلن جاهزة قبل اكتمال الحساب، التأكيد، PDF، والمدفوعات.",
    }
    if data in help_text:
        await query.edit_message_text(help_text[data], reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("↩️ القائمة", callback_data="menu")]]))
...
def build_telegram_application(token: str, owner_id: int, session_factory) -> Application:
    ...
    application.add_handler(CommandHandler("addlease", add_lease))
    application.add_handler(CommandHandler("meter", add_meter))
